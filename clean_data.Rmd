...
chs_sas = chs %>%
  mutate(insure5 = ifelse(insure5 %in% c(1,2,3,4,5), insure5, "."),
         ...
         generalhealth = ifelse(generalhealth %in% c(1,2,3,4,5), generalhealth, ".")) %>%
  select(c(newrace, insure5, insured, pcp18, didntgetcare18,generalhealth, imputed_povertygroup, birthsex, agegroup, education, strata, wt19_dual)) %>% 
  mutate(newrace=ifelse(newrace %in% c(4,5), 4, newrace),
         agegroup=ifelse(agegroup %in% c(1,2), 1, agegroup),
         generalhealth = ifelse(generalhealth %in% c(4,5),4, generalhealth),
         imputed_povertygroup=ifelse(imputed_povertygroup %in% c(4,5), 4, imputed_povertygroup))



write_csv(chs_sas, "chs_clean_recode.csv")
```
